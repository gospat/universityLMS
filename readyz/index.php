<?php
///////////////////////////////////////////////////////////////////////////
// ULMS /readyz — Kubernetes readiness probe.
//
// Readiness = "is this pod ready to accept real user traffic?"
// Unlike liveness (/healthz), readiness includes heavy checks:
//   - DB ping (mysqli real connect)
//   - moodledata/cache directory writable (MUC works)
//   - php-error.log writable (error recording works)
//   - ulms_safe_error_handler.php file exists + is_readable
//   - extra: handler class is instantiable via require_once (no parse error)
// If kubelet sees 3 consecutive readiness failures → pod is PULLED OUT of
// the service endpoint pool but NOT restarted (user traffic goes elsewhere).
//
// Response: HTTP 200 JSON {"status":"ok"/"degraded","time":"...","checks":{db,cache,log,handler,boot:true/false},"ref":"<8hex>"}
//           HTTP 503 on any check failure (standard K8s recognition).
///////////////////////////////////////////////////////////////////////////
...
if (is_file($ulmsHandlerPath) && is_readable($ulmsHandlerPath)) {
    ...
    try {
        require_once($ulmsHandlerPath);
        if (class_exists('local_ulms_auth\local\error\ulms_safe_error_handler', false)) {
            $ulmsChecks['boot'] = true;
        }
    } catch (\Throwable $_e) {
        $ulmsChecks['boot'] = false;
    }
}
