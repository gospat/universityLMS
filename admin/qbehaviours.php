...
if (!$systemcontext instanceof context) {
    throw new coding_exception('System context must extend the base context type.');
}
...
function question_behaviour_enable_disable_icons(string $behaviour, bool $enabled): string {
    if ($enabled) {
...
function question_behaviour_icon_html(string $action, string $behaviour, string $icon,
        string $alt, ?string $tip): string {
    global $OUTPUT;
